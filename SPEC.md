...
version: 5
describes: 0.5.0
status: DRAFT — leg 1 of the 0.5.0 change set; the release cut is at leg 2
---
...
|---|---|---|---|
| `kami <index>` | single-kami vitals (HP now/total/%, state, cooldown, output) plus the leveling block: level, banked `xp`, `xpRequired`, `levelUpReady`, `levelUpBlockedBy` when it is not, and unspent `skillPoints` | mirror *or* stateless | G3.a, G3.d, G2.a, G2.b, G6.a |
| `account <index\|name>` | account identity, room, stamina (current + total), reputation, owned kamis; bio only under `--prose`; the account's own native gas balance when the daemon has a reachable RPC | mirror (+RPC for the gas block only) | G3.a, G6.a, G6.d |
| `party [accountIndex]` | an account's kamis with full vitals, incl. the leveling block; row list capped at 50 with `kamisTotal`/`kamisServed` served, ordered by kami index; `--full` lifts the cap | mirror | G3.c, G7.a |
| `roster [accountIndex]` | compact roster: one line per kami (index, state, `[hp, hpTotal]`) + the account's room | mirror | G3.a, G3.f, G7.a |
| `node <index> [attacker] [--with-vitals] [--full]` | node + ACTIVE harvests, compact rows capped at 50 with `harvestsTotal`/`harvestsServed`, ordered by kami index; with the flag, occupant vitals (incl. level and the leveling block) and the pairwise liquidation preview, whose `eligible: false` names its `reason`; `--full` lifts the cap and restores names/ids/description | mirror | G3.b, G6.a, G6.b, G7.a |
...
| `status` | sync state, block lag, effective config + its source level (including `enrich`), tripwire counters, per-feed degradation, and `rpcReads` — the health of the query layer's own chain reads | daemon | G3.a, G3.e, G5.c |
